...
const uint8_t *report_desc[] = {desc_hid_keyboard_report, desc_hid_mouse_report};

#define CONFIG_TOTAL_LEN (TUD_CONFIG_DESC_LEN + 2 * TUD_HID_DESC_LEN)

#define KEYBOARD_DESC_INDEX 1
#define MOUSE_DESC_IDEX 2
...
void core1_main()
{
    ...
    static pio_usb_configuration_t config = {
        SKM_PIO_USB_PIN_DP, PIO_USB_TX_DEFAULT, PIO_SM_USB_TX_DEFAULT,
...
void skmhid_move_mouse(int8_t x, int8_t y)
{
    ...
    endpoint_t *ep = pio_usb_get_endpoint(usb_device, MOUSE_DESC_IDEX);
...
void skmhid_keyboard_set_key_state(uint8_t *key_codes, int key_code_count)
{
    ...
    {
        keyboard_report.keycode[i] = key_codes[i];
    }

    endpoint_t *ep = pio_usb_get_endpoint(usb_device, KEYBOARD_DESC_INDEX);
    pio_usb_set_out_data(ep, (uint8_t *)&keyboard_report, sizeof(keyboard_report));
}

bool skmhid_is_connected()
{
    return usb_device != NULL;
}
